...
//  node_status_pub_ptr_->CHECK_RATE("/topic/rate/points_raw/slow",8,5,1,"topic points_raw subscribe rate low.");
  ...
  ClassifyPointCloud(radial_ordered_clouds, ground_indices, no_ground_indices); //分割地面点和非地面点
  ...
  ros::Duration duration = ros::Time::now() - t1;
  ROS_INFO("pub groundless_points_pub_ %zu, cost %f s", no_ground_cloud_ptr->size(), duration.toSec());
}
...
void RayGroundFilter::Run()
{
  ...
  radial_dividers_num_ = ceil(360 / radial_divider_angle_);   //将水平方向按照 角度步长， 划分为不同的区间
  ...
  points_node_sub_ = node_handle_.subscribe(input_point_topic_, 1, &RayGroundFilter::CloudCallback, this); //callback中进行函数处理
